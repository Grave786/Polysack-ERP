...
import { Building2, ChevronDown, ShieldCheck, Bell, BellOff, LogOut, Loader2, Check } from 'lucide-react';
...
export default function Topbar() {
    ...
    const handleSelectLocation = (loc) => {
        ...
        setShowFacilityDropdown(false);
        // TODO: Filter global API queries by selected currentFacility ID across all modules
    };

    const displayFacilityName = currentFacility || user?.facilityName || 'Vapi Unit #1 (GIDC Phase 3)';

    return (
        <header className="h-16 bg-sidebar-bg border-b border-sidebar-hover flex items-center justify-between px-6 text-sidebar-text-active font-sans gap-8 shadow-xs">
            {/* Left: Brand Logo & Title (Fixed width, far left) */}
            <div className="flex items-center gap-3 shrink-0">
                <div className="w-8.5 h-8.5 bg-primary rounded-xl flex items-center justify-center font-extrabold text-base text-sidebar-bg shadow-sm">
                    P
                </div>
                <div className="text-base font-bold tracking-tight text-sidebar-text-active hidden sm:block whitespace-nowrap">
                    PolySack <span className="text-primary font-semibold">ERP</span>
                </div>
            </div>
            ...
            <div className="flex items-center gap-4 shrink-0">
                ...
                <div className="relative" ref={facilityRef}>
                    <button
                        type="button"
                        className="hidden lg:flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-sidebar-hover/80 border border-sidebar-hover text-xs font-semibold text-sidebar-text-active hover:border-primary/40 transition-all cursor-pointer shadow-2xs select-none"
                        onClick={handleToggleFacilityDropdown}
                    >
                        <Building2 size={14} className="text-primary shrink-0" />
                        <span className="truncate max-w-[150px]">{displayFacilityName}</span>
                        <ChevronDown size={13} className="text-sidebar-text ml-0.5 shrink-0" />
                    </button>
                ...
                {/* Role Badge Pill */}
                <div className="hidden md:flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-primary/15 border border-primary/40 text-primary text-xs font-bold shadow-2xs select-none">
                    <ShieldCheck size={14} />
                ...
                <div className="relative" ref={notificationRef}>
                    <button
                        ...
                        className="p-2 text-sidebar-text hover:text-sidebar-text-active rounded-xl hover:bg-sidebar-hover transition-all cursor-pointer relative"
                        title="Notifications"
                    >
                        <Bell size={18} />
                        {notificationCount > 0 && (
                    ...
                    {showNotificationDropdown && (
                        <div className="absolute right-0 mt-2 w-72 bg-card-bg border border-border shadow-2xl rounded-xl z-50 p-4 font-sans text-xs text-text-main">
                            ...
                            {/* TODO: Connect to backend GET /api/notifications endpoint when available */}
                        </div>
                    )}
                </div>

                {/* User Info Block & Avatar */}
                <div className="flex items-center gap-3 border-l border-sidebar-hover pl-4">
                    <div className="w-8.5 h-8.5 rounded-full bg-primary text-sidebar-bg font-extrabold text-xs flex items-center justify-center shadow-xs shrink-0 select-none">
                        {userInitials}
                    </div>

                    <div className="hidden sm:flex flex-col text-left leading-tight select-none">
                        <span className="text-xs font-bold text-sidebar-text-active truncate max-w-[130px]">
                            {user?.name || 'Authorized User'}
                        </span>
                        <span className="text-[10px] text-sidebar-text font-medium truncate max-w-[130px]">
                            {roleDisplayName}
                        </span>
                    </div>
                    ...
                    <button
                        onClick={logout}
                        className="p-1.5 rounded-lg bg-sidebar-hover text-sidebar-text hover:text-rose-400 border border-sidebar-hover transition-all ml-1 cursor-pointer"
                        title="Sign Out"
                    >
                        <LogOut size={14} />
                    </button>
